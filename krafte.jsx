    ...
    >
      <div className="grid-2">
        <div>
          ...
          <input
            ...
            autoFocus
            value={k.funkruf || ""}
            onChange={(ev) => set({ funkruf: ev.target.value })}
            placeholder="Florian Musterstadt 1-44-1"
          />
        </div>
        <div>
      ...
      <div className="grid-3">
        <div>
          ...
          <select
            className="select"
            value={k.fach || "feuerwehr"}
            onChange={(ev) => set({ fach: ev.target.value })}
          >
            {FACH_OPTIONS.map(([v, l]) => (
        ...
        <div>
          ...
          <select
            className="select"
            value={k.form || "fahrzeug"}
            onChange={(ev) => set({ form: ev.target.value })}
          >
            {FORM_OPTIONS.map(([v, l]) => (
              <option key={v} value={v}>
                {l}
              </option>
            ))}
        ...
        <div>
          ...
          <select
            className="select"
            value={k.groesse || "gruppe"}
            onChange={(ev) => set({ groesse: ev.target.value })}
          >
            {GROESSE_OPTIONS.map(([v, l]) => (
              <option key={v} value={v}>
                {l}
              </option>
            ))}
      ...
      <div className="grid-3">
        <div>
          ...
          <input
            className="input mono"
            value={k.text || ""}
            onChange={(ev) => set({ text: ev.target.value.toUpperCase() })}
            placeholder="LZ, HLF, RTW …"
            maxLength={6}
          />
        </div>
        <div>
          <label className="label">Personalstärke</label>
          <div className="kraft-staerke-grid">
            <div>
              <div className="label" style={{ marginBottom: 4, fontSize: 10 }}>
                F
              </div>
              <input
                className="input mono kraft-staerke-input"
                type="number"
                min="0"
                inputMode="numeric"
        ...
        <div>
          ...
          <select
            className="select"
            value={k.fms || "2"}
            onChange={(ev) => set({ fms: ev.target.value })}
          >
            {Object.entries(FMS_LABELS).map(([v, l]) => (
              <option key={v} value={v}>
                {v.toUpperCase()} – {l}
              </option>
            ))}
      ...
      <div>
        ...
        <input
          className="input"
          value={k.bemerkung || ""}
          onChange={(ev) => set({ bemerkung: ev.target.value })}
          placeholder="z. B. Atemschutz 2 Trupps, Sonderlöschmittel …"
        />
      </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <input
              ...
              onChange={(ev) =>
                set({ einsatzSeit: fromLocalInputValue(ev.target.value) })
              }
              style={{ flex: 1 }}
            />
            <button
              className="btn"
              type="button"
              onClick={() => set({ einsatzSeit: nowISO() })}
              title="Auf jetzt setzen"
            >
              jetzt
            </button>
            {k.einsatzSeit && (
          ...
          {k.einsatzSeit && (
            <div className="hint-line">
              Läuft seit {dauerStr(k.einsatzSeit)}
            </div>
          )}
        </div>
        <div>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <input
              ...
              onChange={(ev) =>
                set({ eaSince: fromLocalInputValue(ev.target.value) })
              }
              style={{ flex: 1 }}
            />
            <button
        ...
        <div className="kraft-meta-row">
          ...
          >
            ...
            <span className="time-val">
              {showTime ? dauerStr(kraft.eaSince) : "0 min"}
            </span>
...
function KraefteTab({ data, setData, onPrint, ablThreshold = 6 }) {
  ...
  const poolList = useMemo(
    () => Object.values(kraefte).filter((k) => !k.ea),
    [kraefte],
  );
  const buckets = useMemo(() => {
    const m = {};
    Object.values(eas).forEach((ea) => {
      m[ea.id] = [];
    });
    Object.values(kraefte).forEach((k) => {
      if (k.ea && m[k.ea]) m[k.ea].push(k);
    });
    return m;
  }, [kraefte, eas]);
  ...
  const addEA = () => {
    ...
    setData((d) => ({
      ...d,
      einsatzabschnitte: { ...(d.einsatzabschnitte || {}), [id]: ea },
    }));
  };

  const updateEA = (id, patch) =>
    setData((d) => ({
      ...d,
      einsatzabschnitte: {
        ...(d.einsatzabschnitte || {}),
        [id]: { ...(d.einsatzabschnitte || {})[id], ...patch },
      },
    }));
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      if (!d.started) {
        ...
        ghost.className =
          "drag-ghost " + ghost.className.replace("is-dragging", "");
      ...
      if (target !== d.lastDropTarget) {
        document
          .querySelectorAll("[data-drop].drop-active")
          .forEach((n) => n.classList.remove("drop-active"));
  ...
  // ---- Render --------------------------------------------------------------
  const eaList = Object.values(eas).sort(
    (a, b) => (a.ordnung || 0) - (b.ordnung || 0),
  );
  ...
  const totalPers = Object.values(kraefte).reduce(
    (s, k) => s + staerkeGesamt(k.staerke),
    0,
  );
  const assignedPers = Object.values(kraefte)
    .filter((k) => !!k.ea)
    .reduce((s, k) => s + staerkeGesamt(k.staerke), 0);
      ...
      <div className="panel">
        <div className="panel-head">
          ...
          <div className="right">
            <button
              className="btn sm"
              onClick={addCustomKraft}
              title="Eigene Kraft anlegen"
            >
              <I.plus /> Eigene
            </button>
          </div>
        ...
        <div className="panel-body" data-drop="pool">
          {filteredPool.length === 0 && (
            <div className="ea-empty" style={{ minHeight: 80 }}>
              Pool leer — Kräfte aus Vorlagen anlegen ↓
            </div>
          )}
          {filteredPool.map((k) => (
            <KraftTile
      ...
      <div className="panel">
        <div className="panel-head">
          ...
          <span className="count" style={{ marginLeft: 4 }}>
            Gebunden {assigned}/{total}
          </span>
          {totalPers > 0 && (
            <span className="count" style={{ marginLeft: 4 }}>
              {assignedPers}/{totalPers} Pers.
            </span>
          )}
          <div className="right">
            <button
              className="btn sm"
              onClick={onPrint}
              title="Kräfteübersicht drucken / als PDF speichern"
            >
        ...
        <div className="panel-body" style={{ background: "var(--bg)" }}>
          {eaList.length === 0 ? (
            <div className="empty-state">
              ...
              <p>
                Lege Abschnitte an und ziehe Kräfte aus dem Pool per Drag & Drop
                hinein.
              </p>
              <div style={{ height: 12 }} />
              <button className="btn primary" onClick={addEA}>
                <I.plus /> Ersten Abschnitt anlegen
              </button>
            </div>
          ) : (
            <div className="eas">
                ...
                <div key={ea.id} className="ea">
                  <div className="ea-head">
                    <input
                      ...
                      onChange={(e) =>
                        updateEA(ea.id, { farbe: e.target.value })
                      }
                      title="Farbe wählen"
                      style={{
                        width: 22,
                    ...
                    <input
                      ...
                      value={ea.name}
                      onChange={(e) =>
                        updateEA(ea.id, { name: e.target.value })
                      }
                    />
                    <button
                  ...
                  <div className="ea-sub">
                    <span className="ea-leader-row">
                      ...
                      <input
                        ...
                        value={ea.leiter || ""}
                        onChange={(e) =>
                          updateEA(ea.id, { leiter: e.target.value })
                        }
                      />
                    </span>
                  </div>
                    ...
                    return (
                      <div className="ea-stats">
                        <span className="ea-stat">
                          ...
                          <span className="ea-stat-lbl">
                            {totalUnits === 1 ? "Einheit" : "Einheiten"}
                          </span>
                        </span>
                        <span className="ea-stat">
...
function VorlagenBrowser({ fuehrungsstufe, onPick }) {
  ...
  const matchCount = Object.values(filteredByCat).reduce(
    (a, b) => a + b.length,
    0,
  );
        ...
        return (
          <div key={cat.key} className="vorl-cat">
            <button
              ...
              onClick={() =>
                setOpenCat((o) => ({ ...o, [cat.key]: !o[cat.key] }))
              }
              type="button"
            >
              ...
              <div className="vorl-list">
                {items.map((t) => (
                  <button
                    ...
                    title={
                      t.label +
                      (t.staerke ? " · " + staerkeAnzeige(t.staerke) : "")
                    }
                    type="button"
                  >
                    <TaktischesZeichen
                    ...
                    <div className="vorl-info">
                      ...
                      {t.staerke && (
                        <div className="vorl-st">
                          {staerkeAnzeige(t.staerke)}
                        </div>
                      )}
                    </div>
                  </button>
