(function (root, factory) {
  ...
  function summarizeKraefte(data) {
    ...
    const byId = {};
    abschnitte.forEach((ea) => { byId[ea.id] = ea; });
    ...
    const poolPersonal = pool.reduce((sum, kraft) => sum + staerkeGesamt(kraft.staerke), 0);
    const zugeordneteKraefte = abschnitte.reduce((sum, ea) => sum + ea.kraefteCount, 0);
    const zugeordnetesPersonal = abschnitte.reduce((sum, ea) => sum + ea.personalCount, 0);
    ...
    return {
      ...
      gesamtPersonal: Object.values(kraefte).reduce((sum, kraft) => sum + staerkeGesamt(kraft.staerke), 0),
      zugeordneteKraefte,
